...
namespace Materias
{
    public partial class Form1 : Form
    {
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            int i = 0;
            while (File.Exists("Cronograma" + (i > 0 ? i + "" : "") + ".png"))
                i++;
            memoryImage.Save("Cronograma" + (i > 0 ? i + "" : "") + ".png", ImageFormat.Png);

        }

        private void webBrowser1_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            if (webBrowser1.DocumentTitle.Equals("Navegación cancelada"))
                webBrowser1.Navigate("http://alumno2.unlam.edu.ar/index.jsp");
            else if (!webBrowser1.Url.ToString().Contains("http://alumno2.unlam.edu.ar/index3.jsp?page=co_oferta_materias"))
            {
                if (!webBrowser1.Url.ToString().EndsWith("http://alumno2.unlam.edu.ar/index.jsp"))
                    webBrowser1.Navigate("http://alumno2.unlam.edu.ar/index3.jsp?page=co_oferta_materias2");
            }
            else
            {
